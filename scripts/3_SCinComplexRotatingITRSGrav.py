...
#поля Земли и неравномерности вращения
#
...
def geoaccel(t, P, coeffs):
    # переход
    cart_coords = CartesianRepresentation(x=P.numcord, unit=u.m, copy=True)
    spher_coords = SphericalRepresentation.from_cartesian(cart_coords)
    GCRS_coords = GCRS(spher_coords, obstime=t0+t*u.s)
    ITRS_coords = GCRS_coords.transform_to(ITRS(obstime=t0+t*u.s))
    P = Position("P(ITRS)", ITRS_coords.x.value, ITRS_coords.y.value, ITRS_coords.z.value)
    ...
    ITRS_gcord = ITRS(x=g.numcord, obstime=t0+t*u.s)
    GCRS_gcord = ITRS_gcord.transform_to(GCRS(obstime=t0+t*u.s))
    cart_gcord = CartesianRepresentation.from_representation(GCRS_gcord)
    g = Acceleration("g", cart_gcord.x.value, cart_gcord.y.value, cart_gcord.z.value)

    return g

# Правые части ОДУ
def SimpleMotionEquation(t, y):
...
print("-------------------------------")
print(V0)
cart_coords = CartesianRepresentation(x=P0.numcord, unit=u.m, copy=True)
spher_coords = SphericalRepresentation.from_cartesian(cart_coords)
GCRS_coords = GCRS(spher_coords, obstime=t0)
ITRS_coords = GCRS_coords.transform_to(ITRS(obstime=t0))
P0_ITRS = Position("P0(ITRS)", ITRS_coords.x.value, ITRS_coords.y.value, ITRS_coords.z.value)
print(f"Положение КА на момент времени {t0} в связанной с Землёй системе координат ITRS")
print(P0_ITRS)
print("--------------------")
